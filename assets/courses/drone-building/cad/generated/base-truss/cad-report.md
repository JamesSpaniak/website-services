# Base kit — truss-arm X with lattice keel (Fusion-frame ideas, parametric) — exact CAD, checks and simulation

Variant `base-truss` · kit **base** · `python -m dronecad build -v base-truss` in 54 s from `bom.json` + `variants.json` (dronecad 0.3.0). Arms: truss 6×4 mm PETG; keel; plate cutouts.
...
| motor leads ≤ 150 mm (swept route) | PASS | longest 52 mm |
...
| arm SF ≥ 2 at max thrust (exact section) | PASS | SF 70.4 |
| ESC ΔT < 60 °C at 50 % throttle | PASS | ΔT 38 °C (enclosed) |
| gust 5 m/s: roll excursion < 10° with BF defaults | PASS | 0.1°, drift 2.49 m |
| print/frame-bottom.stl prints cleanly | PASS | 24.9 g, 1h 24m 37s (OrcaSlicer); print with supports under 1384 mm² of overhang (7.5 %) |
...
| FEA SF ≥ 2 at max thrust (p99 basis) | PASS | max 12.2 MPa (SF 4.1), p99 5.6 MPa |
| first arm mode vs the idle→hover motor band (FEA) | NOTE | 82 Hz = 4914 rpm vs idle 80 Hz – hover 252 Hz — crossed on every spool-up; expect gyro noise there, keep Betaflight RPM filtering on; stiffer arms (truss / CF) push it up |

By-design contacts (fasteners, spokes on pads, wires on pads, hub on shaft): 745 mm³ in 31 pairs.

### Wire routes (swept)

| Wire | Length (mm) | Ø (mm) | Mass (g) |
|---|---:|---:|---:|
| Motor 1 leads (3× 22 AWG) | 52 | 2.4 | 0.36 |
| Motor 2 leads (3× 22 AWG) | 52 | 2.4 | 0.36 |
| Motor 3 leads (3× 22 AWG) | 52 | 2.4 | 0.36 |
| Motor 4 leads (3× 22 AWG) | 52 | 2.4 | 0.36 |
| Battery leads (2× 14 AWG) | 20 | 3.0 | 0.23 |
| RX harness (4 wires) | 61 | 1.6 | 0.20 |

Notes: keel: lattice side walls tie the plates together (stiffness; adds mass, blocks side access)

## Mass and balance (from the solids)

**AUW 218.1 g** · frame + guards 64.2 g · by kind {'frame': 38.3, 'electronics': 59.7, 'hardware': 19.1, 'prop': 7.2, 'guard': 26.0, 'battery': 66, 'wire': 2.0} · CG (-3.3, 0.0, 23.1) mm · CG **1.6 mm above the prop plane** (z 21.5).

Voxel model: AUW 214.4 g, frame + guards 60.5 g, CG z 23.4 mm.

| Component | Mass (g) | Volume (mm³) | Source |
|---|---:|---:|---|
| Frame bottom + arms | 23.9 | 20934 | 20934 mm³ × 1.27 × 0.9 |
...
| Wire: Motor 1 leads (3× 22 AWG) | 0.4 | 224 | swept volume × 1.6 g/cm³ |
| Wire: Motor 2 leads (3× 22 AWG) | 0.4 | 224 | swept volume × 1.6 g/cm³ |
| Wire: Motor 3 leads (3× 22 AWG) | 0.4 | 224 | swept volume × 1.6 g/cm³ |
| Wire: Motor 4 leads (3× 22 AWG) | 0.4 | 224 | swept volume × 1.6 g/cm³ |
| Wire: Battery leads (2× 14 AWG) | 0.2 | 142 | swept volume × 1.6 g/cm³ |
| Wire: RX harness (4 wires) | 0.2 | 123 | swept volume × 1.6 g/cm³ |
| XT30 pigtail | 7.1 | 1152 | BOM |

Inertia about the CG (kg·m²): Ixx 3.36e-04 · Iyy 3.76e-04 · Izz 6.09e-04 · Ixz 1.2e-07
...
|---|---:|---:|---:|---:|
| 0.15 | 5.8 | 60.0 | 500.2 | 5.0 |
| 0.50 | 19.5 | 50.8 | 481.8 | 5.0 |
| 0.85 | 33.1 | 36.7 | 470.6 | 5.0 |

Beam check at the weakest station:

| Load | Force (N) | Stress (MPa) | SF flat print | SF standing print |
|---|---:|---:|---:|---:|
| max thrust | 3.7 | 0.7 | 70.4 | 42.3 |
| crash 5 g on the arm tip | 10.7 | 2.0 | 24.6 | 14.8 |
| crash 10 g | 21.4 | 4.1 | 12.3 | 7.4 |
| crash 20 g | 42.8 | 8.1 | 6.2 | 3.7 |

Tip deflection at max thrust 0.16 mm · first bending mode (beam) 229 Hz vs prop 252 Hz at hover / 667 Hz max.

### FEA — bottom frame (13140 tet10, 76302 free DOF, 4.0 mm mesh, 19 s)

| Case | Max von Mises (MPa) | p99 (MPa) | SF (p99) | Max displacement (mm) | Hotspot (mm) |
|---|---:|---:|---:|---:|---|
| max thrust on all motors | 12.2 | 5.6 | 8.9 | 0.86 | [-2.0, -19.6, 3.5] |
| 10 g vertical crash on one arm tip | 137.5 | 22.1 | 2.3 | 4.94 | [2.0, -19.6, 3.5] |
| 10 g lateral crash on one arm tip | 60.4 | 21.7 | 2.3 | 5.15 | [-2.0, -19.6, 3.5] |

First modes with motors, props and guards lumped on the pads: 82 Hz, 82 Hz, 83 Hz. Max von Mises includes the clamp-edge singularity at the standoffs (hotspot column); the p99 value is the design number. Linear elastic tet10; clamped at the stack footprint; loads spread over pad top nodes; lumped masses for modes. Layer adhesion not modelled — multiply SF by the material's layer_factor for standing prints.

## Flight dynamics

| Item | Fixed-gain reference sim | Betaflight-default PID sim |
|---|---|---|
| Static hover thrust M1–M4 (g) | 57.9 / 51.1 / 57.9 / 51.1 | — |
| 15° roll step peak | 15.0° | 15.0° |
| Return to < 1° | 0.40 s | 0.54 s |
| Side gust 5 m/s: roll peak / drift | — | 0.1° / 2.49 m |
...
|---|---:|---:|---|
| hover (enclosed by keel) | 1.0 | 34 | yes |
| 50 % throttle (enclosed by keel) | 1.1 | 38 | yes |
...
|---|---:|---:|---:|---:|---:|---:|---:|---|
| frame-bottom | 20934 | 0.61 | 16.2 (slicer 24.9) | 70 (slicer 1h 24m 37s) | 7.5 % | 0.0 % | 0 | print with supports under 1384 mm² of overhang (7.5 %) |
...
Mass model vs print model: `print_solidity` 0.9 in bom.json (used for the mass table) vs 0.61 estimated for these slicer settings — the frame will weigh between 16 g and 24 g depending on infill; the slicer says 24.9 g; calibrate with the first test print.
