# Base kit — H frame (two printed rails + center plates) — exact CAD, checks and simulation

Variant `base-h` · kit **base** · `python -m dronecad build -v base-h` in 47 s from `bom.json` + `variants.json` (dronecad 0.3.0). Arms: solid 10×4 mm PETG.
...
| wheelbase 145–168 mm (parts list §4 target 150–160) | PASS | 148.7 mm |
| motor leads ≤ 150 mm (swept route) | PASS | longest 94 mm |
...
| arm SF ≥ 2 at max thrust (exact section) | PASS | SF 7.4 |
| ESC ΔT < 60 °C at 50 % throttle | PASS | ΔT 16 °C |
| gust 5 m/s: roll excursion < 10° with BF defaults | PASS | 0.1°, drift 2.48 m |
| print/frame-bottom.stl prints cleanly | PASS | 20.0 g, 49m 44s (OrcaSlicer) |
...
| FEA SF ≥ 2 at max thrust (p99 basis) | PASS | max 79.3 MPa (SF 0.6), p99 8.7 MPa |
| first arm mode vs the idle→hover motor band (FEA) | PASS | 27 Hz = 1601 rpm vs idle 80 Hz – hover 252 Hz |

By-design contacts (fasteners, spokes on pads, wires on pads, hub on shaft): 761 mm³ in 23 pairs.

### Wire routes (swept)

| Wire | Length (mm) | Ø (mm) | Mass (g) |
|---|---:|---:|---:|
| Motor 1 leads (3× 22 AWG) | 94 | 2.4 | 0.68 |
| Motor 2 leads (3× 22 AWG) | 94 | 2.4 | 0.68 |
| Motor 3 leads (3× 22 AWG) | 94 | 2.4 | 0.68 |
| Motor 4 leads (3× 22 AWG) | 94 | 2.4 | 0.68 |
| Battery leads (2× 14 AWG) | 20 | 3.0 | 0.23 |
| RX harness (4 wires) | 61 | 1.6 | 0.20 |

## Mass and balance (from the solids)

**AUW 218.4 g** · frame + guards 64.5 g · by kind {'frame': 38.6, 'electronics': 59.7, 'hardware': 17.9, 'prop': 7.2, 'guard': 26.0, 'battery': 66, 'wire': 3.2} · CG (-3.1, 0.0, 22.8) mm · CG **1.3 mm above the prop plane** (z 21.5).

Voxel model: AUW 220.5 g, frame + guards 66.6 g, CG z 22.8 mm.

| Component | Mass (g) | Volume (mm³) | Source |
|---|---:|---:|---|
| Frame bottom + arms | 24.2 | 21166 | 21166 mm³ × 1.27 × 0.9 |
...
| Wire: Motor 1 leads (3× 22 AWG) | 0.7 | 423 | swept volume × 1.6 g/cm³ |
| Wire: Motor 2 leads (3× 22 AWG) | 0.7 | 423 | swept volume × 1.6 g/cm³ |
| Wire: Motor 3 leads (3× 22 AWG) | 0.7 | 423 | swept volume × 1.6 g/cm³ |
| Wire: Motor 4 leads (3× 22 AWG) | 0.7 | 423 | swept volume × 1.6 g/cm³ |
| Wire: Battery leads (2× 14 AWG) | 0.2 | 142 | swept volume × 1.6 g/cm³ |
| Wire: RX harness (4 wires) | 0.2 | 123 | swept volume × 1.6 g/cm³ |
| XT30 pigtail | 5.9 | 1152 | BOM |

Inertia about the CG (kg·m²): Ixx 3.27e-04 · Iyy 3.92e-04 · Izz 6.12e-04 · Ixz 7.6e-07
...
|---|---:|---:|---:|---:|
| 0.15 | 6.5 | 40.0 | 53.5 | 2.0 |
| 0.50 | 21.5 | 40.0 | 53.5 | 2.0 |
| 0.85 | 36.5 | 40.0 | 53.5 | 2.0 |
...
| crash 20 g | 42.8 | 77.7 | 0.6 | 0.4 |

Tip deflection at max thrust 1.85 mm · first bending mode (beam) 67 Hz vs prop 252 Hz at hover / 667 Hz max.

### FEA — bottom frame (14433 tet10, 78012 free DOF, 4.0 mm mesh, 25 s)

| Case | Max von Mises (MPa) | p99 (MPa) | SF (p99) | Max displacement (mm) | Hotspot (mm) |
|---|---:|---:|---:|---:|---|
| max thrust on all motors | 79.3 | 8.7 | 5.8 | 3.74 | [-5.3, 24.6, 2.6] |
| 10 g vertical crash on one arm tip | 689.7 | 48.7 | 1.0 | 33.46 | [5.3, -24.6, 2.6] |
| 10 g lateral crash on one arm tip | 151.2 | 15.8 | 3.2 | 5.32 | [5.3, -24.6, 2.6] |

First modes with motors, props and guards lumped on the pads: 27 Hz, 27 Hz, 40 Hz. Max von Mises includes the clamp-edge singularity at the standoffs (hotspot column); the p99 value is the design number. Linear elastic tet10; clamped at the stack footprint; loads spread over pad top nodes; lumped masses for modes. Layer adhesion not modelled — multiply SF by the material's layer_factor for standing prints.

## Flight dynamics

| Item | Fixed-gain reference sim | Betaflight-default PID sim |
|---|---|---|
| Static hover thrust M1–M4 (g) | 57.7 / 51.5 / 57.7 / 51.5 | — |
...
|---|---:|---:|---:|---:|---:|---:|---:|---|
| frame-bottom | 21166 | 0.70 | 18.7 (slicer 20.0) | 72 (slicer 49m 44s) | 0.0 % | 0.0 % | 0 | ok |
...
Mass model vs print model: `print_solidity` 0.9 in bom.json (used for the mass table) vs 0.70 estimated for these slicer settings — the frame will weigh between 19 g and 24 g depending on infill; the slicer says 20.0 g; calibrate with the first test print.
