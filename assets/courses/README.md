...
| File / folder | Purpose |
|---------------|---------|
| `faa_107_course.json` | **Canonical** course `CourseDetails` payload for course **35**. Unit ids are string refs (`"u1"`, `"u111"`, …) matching prod after the unit-refs migration. Includes Ch.1–5 + Ch.8 complete passes and Ch.6–7 + Ch.9–10 partial passes through Aug 5 2026 (**do not combine** `u5`/`u6`), **149 nodes** / **114 leaves**, and 61 `images_url` entries on units 2/3. **Ship this file** via admin `PUT /courses/35` (publish Ch.3+4 together; include reviewed Ch.5–10 edits). |
