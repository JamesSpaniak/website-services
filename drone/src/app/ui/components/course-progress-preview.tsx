...
export default function CourseProgressPreview({ course, onReset }: CourseProgressPreviewProps) {
  ...
  return (
    <div className="p-4 bg-[var(--surface)] border border-[var(--surface-border)] rounded-lg shadow-md h-full flex flex-col">
      <div className="flex-grow">
        <div className="flex items-center justify-between">
          <Link href={`/courses/${course.id}`} className="group">
            <h4 className="text-lg font-bold text-[var(--brand-foreground)] group-hover:text-[var(--brand-primary)]">{course.title}</h4>
          </Link>
          <div className="relative">
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} onBlur={() => setTimeout(() => setIsMenuOpen(false), 150)} className="p-1 rounded-full hover:bg-[var(--comment-secondary-bg)]">
              <EllipsisVerticalIcon className="h-5 w-5 text-[var(--brand-muted)]" />
            </button>
            {isMenuOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-[var(--surface)] border border-[var(--surface-border)] rounded-md shadow-lg py-1 z-10">
                <button onClick={handleReset} className="w-full text-left block px-4 py-2 text-sm text-red-600 hover:bg-red-500/10">
                  Reset Progress
                </button>
              </div>
            )}
      ...
      <div className="mt-4 border-t border-[var(--surface-border)] pt-2">
