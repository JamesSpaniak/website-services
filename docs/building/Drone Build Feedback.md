...
| 7      | Software               | 2        | Go over Betaflight webapp, not all areas need to be covered because they are for fine tuning or modules that are not being used here. Students will likely all be using the same hardware, so teacher can provide settings backup if short on time. |
