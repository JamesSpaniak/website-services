...
| **Sectional chart figures + interactive viewer** — Jacksonville sectional GeoTIFF received (Aug 15 2026); Phase 1: author picks areas → lat/long crop manifest → script crops → existing image pipeline into unit 2/3 leaves; Phase 2 (later): tiled pan/zoom viewer at `/tools/sectional` with deep links from course. Also pending review/swap: `navigation latitude and longitude 3 of 8.png` (replaces u22 slot 3) and `airscraft by numbers 1of 1.png` (replaces both u211 images; drops the 5,500-peak-aircraft stat — confirm intentional; rename typo before upload) | **Planning** | [`tech/sectional-chart-experience-plan.md`](tech/sectional-chart-experience-plan.md) · [`workflows/tech/course-images.md`](../workflows/tech/course-images.md) |
...
| **Honor the author's END OF UNIT QUIZ/TEST blocks in unit quizzes** (Aug 8 2026 tester feedback) — import flattens the sheet's section / end-of-section / unit-test blocks into one flat `priority: 2` pool, so the 25-Q unit quiz samples randomly across all of u1 instead of guaranteeing the designated test questions. Fix: parse block markers in `scripts/build_unit_level_questions.py`, tag unit-test rows `priority: 1` (generator already fills P1 first), upgrade kept duplicates when the dropped copy was in a test block (rows 281/282/285 dedup to 170–172), and raise unit quiz `question_count` (25 → ~35–40) so section questions still rotate in for repetition. Author to confirm the 3 cross-category rows inside the regs test block (287→u5, 290/295→u9) | **Open** | [`assets/courses/faa-107/questions/faa_107_questions_unit_level_review.csv`](../assets/courses/faa-107/questions/faa_107_questions_unit_level_review.csv) · `drone/src/app/ui/components/unit.tsx` |
...
| **Move Terraform state to an S3 backend + DynamoDB lock** — `terraform/providers.tf` has no backend block, so state is the local `terraform/terraform.tfstate` committed to git (last refreshed Aug 5 2026). Only one machine can safely hold the newest state, there is no lock, and every cloud/CI deploy needs a manual state handoff. Bootstrap bucket + lock table in Terraform, `terraform init -migrate-state`, then drop the state file from git and restore the `.gitignore` rule | **Open — blocks cloud/CI deploys** | [`../workflows/tech/deploy-from-cloud.md`](../workflows/tech/deploy-from-cloud.md) · `terraform/providers.tf` |
| **Fix or retire `.github/workflows/deploy.yml`** — fires on every push to `main` and has failed on all 15 runs; latest dies at `Could not assume role with OIDC: Request ARN is invalid` (`secrets.AWS_ACCOUNT_ID` unset). Also drifted from `pipeline.sh`: `personal-site-*` ECR repos instead of `droneedge-dev-*`, repo-root build context, `terraform apply` with no `-var-file` and no `cloudfront_signing_public_key_pem`. Fix = create `GitHubActionsDeployRole`, set the secret, and rewrite the job around `./pipeline.sh --env dev` on `workflow_dispatch` (after the S3 backend lands). Gives a one-tap mobile deploy | **Open — currently red on every merge** | [`../workflows/tech/deploy-from-cloud.md`](../workflows/tech/deploy-from-cloud.md) · `.github/workflows/deploy.yml` |
