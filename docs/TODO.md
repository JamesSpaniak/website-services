...
| **Secret values are unmonitored for 6 of 8 secrets** — only `db_credentials` and `test_user_password` have terraform-managed values; a value rotated or cleared in the console on the Stripe/JWT/admin-seed/Grafana/CloudFront secrets is invisible to `terraform plan` and to `reconcile-state.sh`. Decide whether to bring more values under Terraform (they would then sit in state) or add a shallow "secret has a value" health check | **Open — decide at the prod cutover** | **Secret rotation** below · `scripts/reconcile-state.sh` |
| **OIDC role + `AWS_DEPLOY_ROLE_ARN` secret for `deploy.yml`** — the workflow is rewritten (manual dispatch, wraps `pipeline.sh`, plan-only default) but cannot authenticate. Create an IAM role GitHub can assume, scoped to what `pipeline.sh` calls (ECR, ECS, Secrets Manager, CloudFront, S3 state, plus whatever `terraform apply` touches), then set the repo secret. Gives a one-tap mobile deploy | **Open — workflow inert until both exist** | [`../workflows/tech/deploy-from-cloud.md`](../workflows/tech/deploy-from-cloud.md) · `.github/workflows/deploy.yml` |
| **Scoped IAM deploy user for cloud sessions** — cloud environment variables are readable by anyone using the environment and AWS SSO is unsupported there, so a deploy from a cloud session needs a dedicated static-key IAM user limited to what `pipeline.sh` calls (ECR, ECS, Secrets Manager, CloudFront, Terraform-managed resources), on a rotation schedule. Not admin keys | **Open** | [`../workflows/tech/deploy-from-cloud.md`](../workflows/tech/deploy-from-cloud.md) |
...
*Every credential in the current stack is dev/test-only and nothing in it is worth stealing today, so this is scheduled work, not an incident. It was also never published: no state file reached any remote branch (see [`TODO_COMPLETED.md`](TODO_COMPLETED.md) 2026-09-22).*

The plan is to rotate **into prod** rather than in place — a `--env prod` stack gets its own `droneedge-*` Secrets Manager entries, separate from the `droneedge-dev-*` ones, which then stay test-only for good. Separate state keys already exist; the rest of the split is [`tech/environment-split-plan.md`](tech/environment-split-plan.md).

| Item | Detail |
|------|--------|
| **DB master password** (**C3**) | Plaintext in `terraform/database.tf`. Replace with `random_password` → `aws_secretsmanager_secret_version`, apply, then redeploy immediately so ECS tasks re-pull. Aurora password change needs no restart; the risk is only the window where a recycled task holds the old value |
| **JWT secret** | Rotating invalidates every access + refresh token at once — **every signed-in user is logged out**. Schedule it, do not do it silently |
| **CloudFront signing key** | Swap the key group's public key and the Secrets Manager private key in one change, or URLs already signed break mid-playback. Low-traffic window |
| **Stripe** | Two keys from the same account, not two accounts — Stripe issues test and live keys natively, so dev keeps `sk_test_*` and prod gets `sk_live_*`. Roll the new key in first, verify a checkout + webhook, then revoke the old one. Webhook signing secrets are per-endpoint, so prod needs its own (**PA41**) |
| **`test_user_password`** | Dev-only fixture, already flagged weak in `dev.tfvars`. Rotate or drop with the split |
| **Also in git** | `terraform/env/{dev,prod}.tfvars` and `terraform/secrets_stripe.tf`. Decide whether real prod values ever land in the repo — they should not; prefer a `-var` at apply time or a Secrets Manager read |

### App Review — open / partial only

| ID | Item | Status |
|----|------|--------|
| **C3** | Hardcoded DB password in `terraform/database.tf` | **Open** — `random_password` + Secrets Manager; see **Secret rotation** above |
...
*Last reviewed: Sep 22 2026 — Terraform state moved to S3 (migration still to run once from the laptop); `deploy.yml` rewritten as manual dispatch; NAT gateway row closed as already shipped Aug 13; state-in-git exposure re-measured as never published, so secret rotation is deferred to the prod cutover rather than treated as an incident. Sep 17: money-model gaps listed as TODO; $29-era CAC / $69 bundle retired (retail is $129). Sep 12: SpeedyBee smoke stopper; Video SKU parked. Sep 11: parts list v3. Sep 10: Money model. Sep 9: parts list v2.*
