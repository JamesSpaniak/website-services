...
Same routed-tab pattern as `/admin`: `app/manager/layout.tsx` → **`ManagerShell`** (`app/manager/shell.tsx`) wraps `ManagerGuard` + org fetch (`getOrganizationDetails`) + `PageShell` + `DashboardTabs`, and provides the org to tab pages via the **`useManagerOrg()`** context (`refreshOrg` re-fetches header seat counts after membership changes). `/manager` redirects to `/manager/overview`.
