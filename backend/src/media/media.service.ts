...
export class MediaService {
  ...
  async generatePresignedUploadUrl(
    ...
    this.logger.log(
      `Presigned URL for ${key} (max ${effectiveMaxBytes} bytes)`,
    );
  ...
  async deleteMedia(key: string): Promise<void> {
    ...
    void this.invalidateCloudFrontPaths([`/${key}`]).catch((err) =>
      this.logger.warn(
        `CF invalidation (delete ${key}): ${(err as Error).message}`,
      ),
    );
  ...
  async deleteMultipleMedia(keys: string[]): Promise<void> {
    ...
    void this.invalidateCloudFrontPaths(keys.map((k) => `/${k}`)).catch((err) =>
      this.logger.warn(
        `CF invalidation after bulk delete: ${(err as Error).message}`,
      ),
    );
  ...
  async invalidateCloudFrontPaths(paths: string[]): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `CloudFront invalidation failed: ${(err as Error).message}`,
      );
    }
  }
  ...
  async listMedia(
    ...
    { key: string; publicUrl: string; lastModified?: Date; size?: number }[]
  > {
