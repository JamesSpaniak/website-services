...
export class SignedUrlService {
    ...
    signedVideoCookies(): Record<string, string> | null {
        if (!this.enabled) {
            if (process.env.NODE_ENV === 'production') {
                this.logger.error('CloudFront signing not configured in production — refusing to issue video cookies');
                throw new InternalServerErrorException('Media signing is not configured.');
            }
            this.logger.warn('CloudFront signing not configured — skipping signed video cookies (non-production only)');
            return null;
        }
            ...
            Statement: [
                {
                    ...
                    Condition: {
                        DateLessThan: {
                            'AWS:EpochTime': Math.floor(Date.now() / 1000) + this.cookieTtlSeconds,
                        },
                    },
                },
            ],
