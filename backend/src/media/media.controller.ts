...
export class MediaController {
  ...
  @ApiOperation({
    summary:
      "Generate a presigned upload URL for the current user's profile picture",
  })
  @ApiResponse({
    status: 201,
  ...
  async getProfilePictureUrl(
    ...
    this.logger.log(
      `Generating profile picture presigned URL for user ${userId}`,
    );
  ...
  async getPresignedUrl(
    ...
    this.logger.log(
      `Generating presigned URL for ${dto.filename} in ${dto.folder}`,
    );
    return this.mediaService.generatePresignedUploadUrl(
      dto.filename,
      dto.contentType,
  ...
  @ApiOperation({
    summary: 'Initiate a multipart upload for large files (Admin only)',
  })
  ...
  @ApiOperation({
    summary: 'Get a presigned URL for a single multipart part (Admin only)',
  })
  @Post('multipart/part-url')
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(Role.Admin)
  async getPartUrl(@Body() dto: MultipartPartUrlDto) {
    const url = await this.mediaService.generatePartPresignedUrl(
      dto.key,
      dto.uploadId,
      dto.partNumber,
    );
  ...
  async completeMultipart(@Body() dto: CompleteMultipartDto) {
    await this.mediaService.completeMultipartUpload(
      dto.key,
      dto.uploadId,
      dto.parts,
    );
  ...
  @ApiOperation({
    summary: 'Preview orphaned media keys (dry run, Admin only)',
  })
  @Get('orphans')
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(Role.Admin)
