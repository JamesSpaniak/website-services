...
export class CourseController {
  ...
  @ApiResponse({
    status: 200,
    description: 'A list of simplified course details.',
    type: [CourseDetails],
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Public course marketing payload.',
    type: CourseDetails,
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Full course details with user progress.',
    type: CourseDetails,
  })
  ...
  async getCourseById(
    ...
    const [details, questionCounts] = await Promise.all([
      this.progressService.getCourseWithProgress(
        req.user.userId,
        id,
        hasAccess,
      ),
  ...
  @Roles(Role.Admin)
  async createCourse(@Body() course: CourseDetails): Promise<Course> {
  ...
  @ApiResponse({
    status: 200,
    description:
      'Playable video URL for the unit (signed cookies set for HLS).',
  })
  @ApiResponse({
    status: 403,
