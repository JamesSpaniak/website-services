...
export class AuditController {
  ...
  @ApiOperation({
    summary: "Get the current user's activity history and login streak",
  })
  @Get('my')
  async getMyActivity(
    @Request() req,
  ...
  @ApiOperation({
    summary: "Get a student's activity log (manager or admin only)",
  })
  @Get('users/:userId')
    ...
    if (caller.role !== Role.Admin) {
      ...
      if (!callerOrg || callerOrg.role !== OrgRole.Manager) {
        throw new ForbiddenException(
          'Only managers can view student activity.',
        );
      }
