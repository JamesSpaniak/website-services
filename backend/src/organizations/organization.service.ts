...
export class OrganizationService {
  ...
  async createOrganization(
    ...
    if (dto.initial_manager_user_id) {
      initialManager = await this.userRepository.findOne({
        where: { id: dto.initial_manager_user_id },
      });
      if (!initialManager) {
        throw new BadRequestException(
          `User with ID ${dto.initial_manager_user_id} not found.`,
        );
      }
    } else if (dto.initial_manager_email) {
      initialManager = await this.userRepository.findOne({
        where: { email: dto.initial_manager_email },
      });
      if (!initialManager) {
        throw new BadRequestException(
          `User with email ${dto.initial_manager_email} not found.`,
        );
      }
    }
  ...
  async getAllOrganizations(): Promise<OrganizationResponse[]> {
    ...
    for (const org of orgs) {
      const [memberCount, managerCount, courseCount] = await Promise.all([
        ...
        this.memberRepository.count({
          where: { organizationId: org.id, role: OrgRole.Manager },
        }),
        this.getOrgCourseCount(org.id),
      ]);
      results.push(
        this.toOrgResponse(org, memberCount, managerCount, courseCount),
      );
    }
  ...
  async getOrganization(id: number): Promise<OrganizationResponse> {
    ...
    const [memberCount, managerCount, courseCount] = await Promise.all([
      ...
      this.memberRepository.count({
        where: { organizationId: id, role: OrgRole.Manager },
      }),
  ...
  async addMember(
    ...
    if (existingMembership) {
      if (existingMembership.organizationId === orgId) {
        throw new BadRequestException(
          `${user.username} is already a member of this organization.`,
        );
      }
      throw new BadRequestException(
        `${user.username} is already a member of another organization.`,
      );
    }
  ...
  async validateAndConsumeInviteCode(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (
        inviteCode.email &&
        inviteCode.email.toLowerCase() !== userEmail.toLowerCase()
      ) {
        throw new BadRequestException(
      ...
      if (inviteCode.role === OrgRole.Member) {
        ...
        if (currentStudentCount >= org.max_students) {
          throw new BadRequestException(
            'This organization has reached its student seat limit.',
          );
        }
      }
