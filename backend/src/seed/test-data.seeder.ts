 ...
 */
export async function seedTestData(dataSource: DataSource, logger: Logger): Promise<void> {
    const rawPassword = process.env.TEST_USER_PASSWORD;
    if (!rawPassword) {
        logger.warn('SEED_TEST_DATA is enabled but TEST_USER_PASSWORD is not set; skipping test seed.');
        return;
    }
    ...
    logger.log(`Test fixtures seeded (testadmin, testuser, organization "${orgName}").`);
}
